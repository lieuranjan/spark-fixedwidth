      ...
      if (headerLen == schemaSize) {
        ...
        while (errorMessage.isEmpty && i < headerLen) {
          ...
          if (nameInHeader != nameInSchema) {
            errorMessage = Some(
              s"""|Txt header does not conform to the schema.
                  | Header: ${columnNames.mkString(", ")}
                  | Schema: ${fieldNames.mkString(", ")}
                  |Expected: ${fieldNames(i)} but found: ${columnNames(i)}
                  |Txt file: $fileName""".stripMargin)
          }
          i += 1
        }
      } else {
        errorMessage = Some(
          s"""|Number of column in Txt header is not equal to number of fields in the schema:
...
object TextInputFixedWidthDataSource extends FixedWidthDataSource {
  ...
  override def readFile(
    ...
    if (hasHeader) {
      ...
      // be not extracted.
      FixedWidthUtils.extractHeader(lines, parser.options).foreach { header =>
        ...
        FixedWidthDataSource.checkHeaderColumnNames(
          schema,
          columnNames,
          file.filePath,
  ...
  /**
   * Infers the schema from `Dataset` that stores string records.
   */
  def inferFromDataset(
                        ...
                        parsedOptions: FixedWidthOptions): StructType = {
    val positionalParser = new FixedWidthParser(parsedOptions.asParserSettings)
    maybeFirstLine.map(positionalParser.parseLine(_)) match {
      case Some(firstRow) if firstRow != null =>
        val caseSensitive = sparkSession.sessionState.conf.caseSensitiveAnalysis
        val header = makeSafeHeader(firstRow, caseSensitive, parsedOptions)
        val sampled: Dataset[String] = FixedWidthUtils.sample(ds, parsedOptions)
        val tokenRDD = sampled.rdd.mapPartitions { iter =>
          val filteredLines = FixedWidthUtils.filterCommentAndEmpty(iter, parsedOptions)
          val linesWithoutHeader =
            FixedWidthUtils.filterHeaderLine(filteredLines, maybeFirstLine.get, parsedOptions)
          val parser = new FixedWidthParser(parsedOptions.asParserSettings)
          linesWithoutHeader.map(parser.parseLine)
        }
        FixedWidthInferSchema.infer(tokenRDD, header, parsedOptions)
      case _ =>
  ...
  override def infer(
    ...
    fixedWidth.flatMap { lines =>
      ...
      UnivocityParser.tokenizeStream(
        ...
        new FixedWidthParser(parsedOptions.asParserSettings))
    }.take(1).headOption match {
      case Some(firstRow) =>
        val caseSensitive = sparkSession.sessionState.conf.caseSensitiveAnalysis
        val header = makeSafeHeader(firstRow, caseSensitive, parsedOptions)
        val tokenRDD = fixedWidth.flatMap { lines =>
          UnivocityParser.tokenizeStream(
        ...
        val sampled = FixedWidthUtils.sample(tokenRDD, parsedOptions)
        FixedWidthInferSchema.infer(sampled, header, parsedOptions)
      case None =>
