 ...
 * @param options        Configuration options for a FixedWidth parser.
 */
class UnivocityParser(
                       dataSchema: StructType,
                       requiredSchema: StructType,
                       val options: FixedWidthOptions) extends Logging {
  ...
  def this(schema: StructType, options: FixedWidthOptions) = this(schema, schema, options)
  ...
  val tokenizer = {
    ...
    // When to-be-parsed schema is shorter than the to-be-read data schema, we let Univocity Fixed
    ...
    new FixedWidthParser(parserSetting)
  }
  ...
  def makeConverter(
                     ...
                     options: FixedWidthOptions): ValueConverter = dataType match {
  ...
  private def nullSafeDatum(
                             ...
                             name: String,
                             nullable: Boolean,
                             options: FixedWidthOptions)(converter: ValueConverter): Any = {
  ...
  /**
   * Parses a single fixedWidth string and turns it into either one resulting row or no row (if the
  ...
  private def convert(tokens: Array[String]): InternalRow = {
    if (tokens == null) {
      throw BadRecordException(
        () => getCurrentInput,
        () => None,
        new RuntimeException("Malformed fixedWidth record"))
      ...
      try {
        ...
        case NonFatal(e) =>
          ...
          // fixedWidth reader doesn't support partial results. All fields other than the field
...
private[fixedwidth] object UnivocityParser {

  /**
   * Parses a stream that contains fixedWidth strings and turns it into an iterator of tokens.
   */
  def tokenizeStream(
                      inputStream: InputStream,
                      shouldDropHeader: Boolean,
                      tokenizer: FixedWidthParser): Iterator[Array[String]] = {
    convertStream(inputStream, shouldDropHeader, tokenizer)(tokens => tokens)
  }

  /**
   * Parses a stream that contains fixedWidth strings and turns it into an iterator of rows.
   */
  def parseStream(
                   inputStream: InputStream,
                   shouldDropHeader: Boolean,
  ...
  /**
   * Parses an iterator that contains fixedWidth strings and turns it into an iterator of rows.
   */
  def parseIterator(
    ...
    val filteredLines: Iterator[String] = FixedWidthUtils.filterCommentAndEmpty(lines, options)
